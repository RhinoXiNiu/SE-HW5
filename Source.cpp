...
int SMTX_print(SMatrixType A[])
{
	...
	int SMTX_add(SMatrixType A[], SMatrixType B[], SMatrixType C[])
	{
		...
		{
			if (A->R[i].row > B->R[j].row || (A->R[i].row == B->R[j].row && A->R[i].col > B->R[j].col))//L171、L173白話文: 如果A和B確定是不同的位置的話
				C->R[k++] = B->R[j++];
			else if (A->R[i].row < B->R[j].row || (A->R[i].row == B->R[j].row && A->R[i].col < B->R[j].col))//L171、L173白話文: 如果A和B確定是不同的位置的話
				C->R[k++] = A->R[i++];
			else//白話文: A的位置和B的位置相同 ->就需要做相加//
			{
	...
	int SMTX_subtract(SMatrixType A[], SMatrixType B[], SMatrixType C[])
	{
		...
		while (i <= A->R[0].value && j <= B->R[0].value)
		{
			if (A->R[i].row > B->R[j].row || (A->R[i].row == B->R[j].row && A->R[i].col > B->R[j].col))//沒問題 //仿L171~L172的「照抄B」邏輯，但因為是相減，而此項A為零，所以需冠上負號
			{
				C->R[k].row = B->R[j].row;
				C->R[k].col = B->R[j].col;
				sum = B->R[j].value;
			...
			else if (A->R[i].row < B->R[j].row || (A->R[i].row == B->R[j].row && A->R[i].col < B->R[j].col))//沒問題 //仿L173~L174的「照抄B」邏輯
				C->R[k++] = A->R[i++];
			else
			{
...
int SMTX_transpose(SMatrixType A[], SMatrixType B[])
{
	...
	if (num_terms > 0)
	{
		for (i = 0; i <= num_col; i++)
		{
			row_terms[i] = 0; //row_terms的row，是指B的row，所以也就是A的col。所以這就是在算A的每個col，各自有多少個entry「，的初始化」
		}
		for (i = 1; i <= num_terms; i++)
		{
			row_terms[A->R[i].col]++; //延續L266，是「真正」在算[A的每個col(a.k.a. B的row)]，各自有多少個entry
		}
		starting_pos[0] = 1;
		for (i = 1; i < num_col; i++)
		{
			starting_pos[i] = starting_pos[i - 1] + row_terms[i - 1]; //計算prefixSum(a.k.a. 累加)
		}
		for (i = 1; i <= num_terms; i++)
		{
...
int SMTX_multiply(SMatrixType A[], SMatrixType B[], SMatrixType C[])
{
	...
	int col_a = A->R[0].col, col_B = B->R[0].col;
